...
%                               
                              if (strcmp(fit_type,'linear') || strcmp(fit_type,'spline') || strcmp(fit_type,'cubic'))
              ...
              for jj=1:mysize(2) %x
                  for kk=1:mysize(4) % Offsets
                      if(SEGMENT(ii,jj,ll)==1 && max(isnan(Z_stack(ii,jj,ll,kk,B1_input_index)))==0 && max(isinf(Z_stack(ii,jj,ll,kk,B1_input_index)))==0 && max(isnan(abs_B1map(ii,jj,ll,B1_input_index)))==0)
                          if (strcmp(fit_type,'linear') || strcmp(fit_type,'spline') || strcmp(fit_type,'cubic'))
%                               Z_stack_corr(ii,jj,ll,kk,:) = interp1(squeeze(abs_B1map(ii,jj,ll,B1_input_index)),squeeze(Z_stack(ii,jj,ll,kk,B1_input_index)),B1_output,inter_method);
