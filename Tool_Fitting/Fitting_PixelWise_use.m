function [map_Cr,map_PCr,map_glycoNOE,res_B0map,Signal_Cr, Signal_PCr, Signal_glycoNOE]=Fitting_PixelWise_use7(images,Mask_all,B0map,Para,result_path)
...
% log : 2024.6.23 add dc for MT and change the pool of 3.5ppm 
...
images_fit_no_p3            = zeros(size(images));
images_fit_no_glycoNOE = zeros(size(images));
images_fit_up_mt            = zeros(size(images));
images_fit_down_mt       = zeros(size(images));
images_fit_base      = zeros(size(images));
images_fit_all         = zeros(size(images));
images_fit_all_up = zeros(size(images));
...
%                     
%                     if any(isnan(zspectrum2fit))
%                         mask(ii, jj) = 0; 
%                         continue
%                     end
                    zspectrum2fit = squeeze(images(ii,jj,B1,:,Slice));
                    S_4ppm = 0.94202;
                    zspectrum2fit =zspectrum2fit./(zspectrum2fit(end)./S_4ppm); 
...
%                           water_direct            |          +2             |               +2.5              |        +3.5              |              MT         |  dc ]
%                            A       w       B          A         w        B           A        w        B        A        w       B          A         w        B     dc                                          
                    x  = [0.7       0        1        0.015     1.9      0.7       0.02     2.5    0.3      0.02     3.3        1.2     0.1     2.8    10     0.0258];
                    lb = [0.05    -0.15    0       0           1.8       0             0      2.3     0           0      3.28       0.5     0         0      2     0.0257];
                    ub = [1        0.15     2      0.05       2.1      0.8         0.05     2.7    0.4      0.03    3.35       2       0.2       5     15    0.0259];
                    
                    pool_num = 5;                       
%                     fit_range_down_back = freq_ppm>0.2 & freq_ppm<=max(freq_ppm); % 0ppm is discarded
                    fit_range_down_back = freq_ppm>0.7 & freq_ppm<=max(freq_ppm); % 0ppm is discarded 
...
%                 
                    images_fit_back_down(ii,jj,B1,sindex(fit_range_down_back),Slice) = signal_cal+peak_cal(:,2)+peak_cal(:,3)+peak_cal(:,4)+peak_cal(:,5);
                    ...
                    downfield_spectrum(freq_ppm<0) = 0;                    
                    ...
                    x  = [ 0.015     1.9      0.7       0.02     2.5    0.3      0.02     3.3        1.2     0.1     2.8    10    -0.05];
                    lb = [    0        1.8      0             0          2.3    0           0    3.28       0.5           0     0      2     -0.1];
                    ub = [ 0.05     2.1      0.8          0.05     2.7    0.4        0.03     3.35       2         0.2      5     15    0.2];
                    
                    pool_num = 4;                

                    fit_range_down = freq_ppm<=max(freq_ppm) & freq_ppm>0.7;
                    ...
                    images_fit_no_p20(ii,jj,B1,sindex(fit_range_down),Slice) = peak_cal(:,2)+peak_cal(:,4)+x(end)+peak_cal(:,3); 
                    images_fit_no_p25(ii,jj,B1,sindex(fit_range_down),Slice) = peak_cal(:,1)+peak_cal(:,4)+x(end)+peak_cal(:,3);
                    images_fit_no_Cr_PCr(ii,jj,B1,sindex(fit_range_down),Slice) = peak_cal(:,3)+x(end) +peak_cal(:,4);
                    images_fit_no_p3(ii,jj,B1,sindex(fit_range_down),Slice) = peak_cal(:,1)+peak_cal(:,2)+x(end)+peak_cal(:,4); 
                    images_fit_base(ii,jj,B1,sindex(fit_range_down),Slice) = x(end)+peak_cal(:,4);
                    
                    images_fit_all(ii,jj,B1,sindex(fit_range_down),Slice) = peak_cal(:,1)+peak_cal(:,2)+peak_cal(:,3)+x(end)+peak_cal(:,4);% xuxi residual spectra
                    ...
                    peak_cal = 0; signal_cal = 0;
                    options = optimset('MaxFunEvals',100000*5, 'MaxIter',1000000,'Display','off');
%                         water_direct        |          +2                    |               +2.5              |        +3.5                |  dc ]
%                         A       w         B       A         w        B           A        w             B           A        w       B      dc    
                    x  = [0.7     0         1     0.02      -1        0.2        0.01     -2.5      0.02         0.2     -3      5     0.0258];
                    lb = [0.05   -0.15   0      0        -1.1       0           0          -2.6        0            0     -5       3     0.0257];                  
                    ub = [1      0.15     3     0.05     -0.9     0.55      0.03      -2.4      0.04          1     -3      20     0.0259];
                    ...
                    images_fit_up_mt(ii,jj,B1,sindex(fit_range_up_back),Slice) = peak_cal(:,4)+x(end);
                    % xuxi updated
                    ...
                    options = optimset('MaxFunEvals',100000*5, 'MaxIter',1000000,'Display','off');
                    % x = [          -1                                 -2.8                                 MT           dc ]  
%                     x  = [  0.02     -1         0.2       0.01     -2.5      0.2             0.2     -3.2      5     0];
%                     lb = [  0       -1.1      0          0          -2.9       0                0      -5        3    -0.05];
%                     ub = [  0.05    -0.9      0.55      0.05      -2.0      0.6             1.0     -3.2    20    0.2];
                    
                    x  = [ 0.02     -1        0.2         0.01     -2.5      0.02               0.2     -3      5     -0.05];
                    lb = [  0        -1.1      0            0          -2.6         0                 0     -5       3    -0.1];
                    ub = [0.05     -0.9     0.55      0.03      -2.4      0.04                1    -3      20    0.1];
                    ...
                    images_fit_all_up(ii,jj,B1,sindex(fit_range_up),Slice) = peak_cal(:,1)+peak_cal(:,2)+peak_cal(:,3)+x(end);
...
%                     sum_glycoNOE = sum_glycoNOE + sum(glycoNOE_signal);
        ...
        for nm = 1:numel(cc.PixelIdxList)
            ...
            tubezspectrum(nm,B1,:,Slice) = squeeze(sum(sum(tube_mask.*images_Corr_z(:,:,B1,:,Slice)))./Tarea);
            tubezspectrum_resd_down(nm,B1,:,Slice) = squeeze(sum(sum(tube_mask.*images_downfield(:,:,B1,:,Slice)))./Tarea); 
            ...
            tubezspectrum_fit_no_p3(nm,B1,:,Slice) = squeeze(sum(sum(tube_mask.*images_fit_no_p3(:,:,B1,:,Slice)))./Tarea);
            
            tubezspectrum_fit_base(nm,B1,:,Slice) = squeeze(sum(sum(tube_mask.*images_fit_base(:,:,B1,:,Slice)))./Tarea);
            tubezspectrum_fit_all(nm,B1,:,Slice) = squeeze(sum(sum(tube_mask.*images_fit_all(:,:,B1,:,Slice)))./Tarea);
            ...
            tubezspectrum_fit_all_up(nm,B1,:,Slice) =  squeeze(sum(sum(tube_mask.*images_fit_all_up(:,:,B1,:,Slice)))./Tarea);
            
            file_name = [result_path,'Z_data.mat'];
             %% plot Z-spectrum
            ...
            plot(freq_ppm(fit_range_down),100*(squeeze(tubezspectrum_fit_base(nm,B1,(fit_range_down),Slice))),'b-', 'linewidth',2);  
            plot(freq_ppm(fit_range_down),100*(squeeze(tubezspectrum_fit_all(nm,B1,(fit_range_down),Slice))),'g-', 'linewidth',2);  
            ...
            set(gca,'ylim', [0 20],'ytick',[0:5:80],'xlim',[-3 4],'xtick',[-8:1:8]);
            ...
            saveas(h, [result_path,'z_B1_', num2str(B1),'S_',num2str(Slice),'.png'], 'png');
            ...
            glycoNOE_area =sum (sum_glycoNOE) 
...
%             Cr
            ...
            set(gca,'fontsize',26)
            set(gca,'XDir','reverse')
            set(gca,'ylim', [-0.5 4],'ytick',[-5:1:25],'xlim',[-3 4],'xtick',[-6:1:6]);
            xlabel('Offset from water (ppm)','fontsize',34)
            ylabel('Signal (%)','fontsize',34)
            grid on;
            h=gcf;
...
save(file_name, 'tubezspectrum','tubezspectrum_resd_down','tubezspectrum_resd_up','tubezspectrum_fit_back_down','tubezspectrum_fit_back_up','tubezspectrum_fit_p20','tubezspectrum_fit_p25','tubezspectrum_fit_no_p20','tubezspectrum_fit_no_p25','tubezspectrum_fit_no_Cr_PCr','tubezspectrum_fit_no_glycoNOE','tubezspectrum_fit_all','tubezspectrum_fit_base','tubezspectrum_fit_all_up','tubezspectrum_fit_no_p3');
